...
public class Invoice {
	...
	public Invoice(double totalPrice, String publication, String invoiceDate, int pubQuantity, int orderNum)
			throws InvoiceHandler {
		invoice_ID = 0;
		try {
...
// Verify that there is a total price as a double greater than 0.00 and less than 1000000000.00.
	...
	public static void validateOrderNum(int quantity) throws InvoiceHandler {
		if (quantity < 1) {
			throw new InvoiceHandler("Order Number has to be more than 0");
		} else if (quantity > 999) {
			throw new InvoiceHandler("Order Number has to be less than 999");
		}
	}
...
// publication name between 3 and 15 characters.
		if (date.isBlank() || date.isEmpty()) {
			...
			throw new InvoiceHandler("Date cannot be less than 3");
		} else if (date.length() > 10) {
			throw new InvoiceHandler("Date cannot be more than 12");
		}
	}
}
